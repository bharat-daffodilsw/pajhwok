        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (categoryData) {
            ...
            for (var i = 0; i < $scope.customSearchData.length; i++) {
                if ($scope.customSearchData[i]["image"]) {
                    $scope.customSearchData[i]["imgurl"] = BAAS_SERVER + "/file/render?filekey=" + $scope.customSearchData[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                } else {
                    /*for default image*/
                    $scope.customSearchData[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                }
            }
            if (!$scope.$$phase) {
                $scope.$apply();
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (categoryData) {
            ...
            for (var i = 0; i < $scope.searchedNews.length; i++) {
                if ($scope.searchedNews[i]["image"]) {
                    $scope.searchedNews[i]["imgurl"] = BAAS_SERVER + "/file/render?filekey=" + $scope.searchedNews[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                } else {
                    /*for default image*/
                    $scope.searchedNews[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                }
            }
            if (!$scope.$$phase) {
                $scope.$apply();
...
pajhwokApp.controller('articleCtrl', function ($scope, $compile, $http, $location, $appService, $rootScope, $routeParams) {
    $scope.getArticleNews = function () {
        if (true) {
            ...
            query.columns = ["_id", "title", "media", "category", "published", "likeby", "likecount", "flaggedby", "description", "commentcount", "clickcount", {"expression":"comment", "columns":["content", "__createdby.firstname", {"expression":"__createdon", "format":"fromnow"}]}, "image", "__createdby.firstname", "activitycount", "clickcount", {"expression":"__createdon", "format":"fromnow"}];
            ...
            $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (topStoriesData) {
                ...
                if ($scope.news[0].sdesc) {
                    $scope.newsdescription = true;
                }
                ...
                for (var i = 0; i < $scope.news.length; i++) {
                    if ($scope.news[i]["image"]) {
                        $scope.news[i]["imgurl"] = BAAS_SERVER + "/file/render?filekey=" + $scope.news[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                    } else {
                        /*for default image*/
                        $scope.news[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                    }
                }
...
pajhwokApp.controller('myMediaCtrl', function ($scope, $compile, $http, $location, $appService, $rootScope) {
    ...
    if ($scope.currentSession) {
        ...
        $scope.getMyMedia = function () {
            ...
            query.columns = columnArray;
            query.$count = 1;
            query.filter = {"__createdby":"{_CurrentUserId}", "published":true};
            ...
            $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (mediaData) {
                ...
                for (var i = 0; i < $scope.myAllMedia.length; i++) {
                    if ($scope.myAllMedia[i]["image"]) {
                        $scope.myAllMedia[i]["imgurl"] = "/rest/file/render?filekey=" + $scope.myAllMedia[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                    } else {
                        /*for default image*/
                        $scope.myAllMedia[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                    }
                }
                if (!$scope.$$phase) {
                    $scope.$apply();
...
pajhwokApp.controller('myunpublishMediaCtrl', function ($scope, $compile, $http, $location, $appService, $rootScope) {
    ...
    if ($scope.currentSession) {
        ...
        $scope.getMyMedia = function () {
            ...
            query.columns = columnArray;
            query.$count = 1;
            query.filter = {"__createdby":"{_CurrentUserId}", "published":{$ne:true}};
            query.max_rows = $scope.rows;

            var queryParams = {query:JSON.stringify(query), "ask":ASK, "osk":OSK, "usk":$scope.currentSession["usk"]};
            var serviceURL = "/rest/data";
            $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (mediaData) {
                $scope.myunpublishAllMedia = mediaData.data;
                $scope.cursor = mediaData.cursor;
                $scope.maxRows = mediaData.$count;
                ...
                for (var i = 0; i < $scope.myunpublishAllMedia.length; i++) {
                    if ($scope.myunpublishAllMedia[i]["image"]) {
                        $scope.myunpublishAllMedia[i]["imgurl"] = "/rest/file/render?filekey=" + $scope.myunpublishAllMedia[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                    } else {
                        /*for default image*/
                        $scope.myunpublishAllMedia[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                    }
                }
                if (!$scope.$$phase) {
                    $scope.$apply();
        ...
        $scope.getMoreMedia = function (cursor) {
            ...
            $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (mediaData) {
                $scope.loadingMedia = false;
                for (var i = 0; i < mediaData.data.length; i++) {
                    $scope.myunpublishAllMedia.push(mediaData.data[i]);
                }
                for (var i = $scope.cursor; i < mediaData.data.length + $scope.cursor; i++) {
                    if ($scope.myunpublishAllMedia[i]["image"]) {
                        $scope.myunpublishAllMedia[i]["imgurl"] = "/rest/file/render?filekey=" + $scope.myunpublishAllMedia[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
            ...
            $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (unpublishedData) {
                ...
                for (var i = 0; i < $scope.unpublishedNews.length; i++) {
                    if ($scope.unpublishedNews[i]["image"]) {
                        $scope.unpublishedNews[i]["imgurl"] = "/rest/file/render?filekey=" + $scope.unpublishedNews[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                    } else {
                        /*for default image*/
                        $scope.unpublishedNews[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                    }
                }
                if (!$scope.$$phase) {
                    $scope.$apply();
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (flaggedData) {
            ...
            for (var i = 0; i < $scope.flaggedNews.length; i++) {
                if ($scope.flaggedNews[i]["image"]) {
                    $scope.flaggedNews[i]["imgurl"] = "/rest/file/render?filekey=" + $scope.flaggedNews[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                } else {
                    /*for default image*/
                    $scope.flaggedNews[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                }
            }
            if (!$scope.$$phase) {
                $scope.$apply();
...
//        mapTypeId: google.maps.MapTypeId.ROADMAP
    ...
    $scope.mapuserfun=function(catmap,langvalue,searchmap)
    {
        ...
        $scope.setZoomMessage($scope.myMap.getZoom());
    }
    ...
    $scope.openMarkerInfo = function(marker,abkeys) {
        ...
        console.log(JSON.stringify(abkeys));
    ...
    $scope.managemarker = function(){
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (markerreturn) {
            $scope.markerreturnvalue=markerreturn;
            if($scope.myMap.getZoom()==6)
            {
                for (var city in citymap2) {
                    ...
                    $scope.showinfo( citymap2[city].center,$scope.markerreturnvalue.$count);
    ...
    $scope.managemarker1 = function(){
        var query = {};
        query.table = "News__pajhwok";
        query.columns =[{"expression":"cityid","aggregate":"count","alias":"count"}];
        query.childs=[{"alias":"cityid","usein":true,"onetoone":true,"relatedcolumn":"_id","query":{"table":"City__pajhwok","columns":["location"]}}]
        ...
        var serviceURL = "rest/data";
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (markerreturn) {
            $scope.markerreturnvalue1=markerreturn.data;
            var citymap3 = {};
...
// for zoom level > 7
    $scope.managemarker2 = function(){
        ...
        var serviceURL = "rest/data";
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (markerreturn) {
            $scope.markerreturnvalue2=markerreturn.data;
            ...
            {   var anonymousFileKey="530835834b43556925e9b1ee";
                if($scope.markerreturnvalue2[i].image){
                    $scope.markerreturnvalue2[i].imageurl ="http://d8.daffodilapps.com/rest/file/render?filekey=" + $scope.markerreturnvalue2[i].image[0].key + '&ask=5301e64492f51ed71842a5d1&osk=530459b8aed74b22457bad37&resize={"width":50}';
                }
                else{
                    $scope.markerreturnvalue2[i].imageurl ="http://d8.daffodilapps.com/rest/file/render?filekey=" + anonymousFileKey + '&ask=5301e64492f51ed71842a5d1&osk=530459b8aed74b22457bad37&resize={"width":50}';
                }
    ...
    $scope.fetchCommonLabel = function () {
        ...
        var params = {"query":JSON.stringify(query), "ask":ASK, "osk":OSK};
        $appService.getDataFromJQuery("/rest/data", params, "GET", "JSON", function (callBackData) {
//            console.log("common label"+ JSON.stringify(callBackData.data));
    ...
    $scope.getCities = function () {
        ...
        var params = {"query":JSON.stringify(query), "ask":ASK, "osk":OSK};
        $appService.getDataFromJQuery("/rest/data", params, "GET", "JSON", function (callBackData) {
            $scope.uploadCities = callBackData.data;
    ...
    $scope.getStates = function () {
        ...
        var params = {"query":JSON.stringify(query), "ask":ASK, "osk":OSK};
        $appService.getDataFromJQuery("/rest/data", params, "GET", "JSON", function (callBackData) {
            $scope.states = callBackData.data;
...
//            query.max_rows = $scope.topstories.length;
        ...
        if ($scope.defaultSelected.selectedState._id && $scope.defaultSelected.selectedState._id != null) {
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (topStoriesData) {
            ...
            for (var i = 0; i < $scope.topStoriesDetail.length; i++) {
                if ($scope.topStoriesDetail[i]["image"]) {
                    $scope.topStoriesDetail[i]["imgurl"] = BAAS_SERVER + "/file/render?filekey=" + $scope.topStoriesDetail[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                } else {
                    /*for default image*/
                    $scope.topStoriesDetail[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                }
            }
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (recentStoriesData) {
            ...
            for (var i = 0; i < $scope.recentStoriesDetail.length; i++) {
                if ($scope.recentStoriesDetail[i]["image"]) {
                    $scope.recentStoriesDetail[i]["imgurl"] = BAAS_SERVER + "/file/render?filekey=" + $scope.recentStoriesDetail[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                }
                else {
                    /*for default image*/
                    $scope.recentStoriesDetail[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                }
            }
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (mostPopularStoriesData) {
            ...
            for (var i = 0; i < $scope.mostPopularStoriesDetail.length; i++) {
                if ($scope.mostPopularStoriesDetail[i]["image"]) {
                    $scope.mostPopularStoriesDetail[i]["imgurl"] = BAAS_SERVER + "/file/render?filekey=" + $scope.mostPopularStoriesDetail[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                }
                else {
                    /*for default image*/
                    $scope.mostPopularStoriesDetail[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                }
            }
            if (!$scope.$$phase) {
                $scope.$apply();
...
//            query.max_rows = $scope.topstories.length;
        ...
        query.cursor = 0;
        query.filter = {"category":{"_id":filterData}, "_id":{"$ne":newsid}, "transid":$scope.selectedTrans};
        query.orders = {"__createdon":"desc"};
        ...
        $appService.getDataFromJQuery(serviceURL, queryParams, "GET", "JSON", function (topStoriesData) {
            ...
            for (var i = 0; i < $scope.similarCategory.length; i++) {
                if ($scope.similarCategory[i]["image"]) {
                    $scope.similarCategory[i]["imgurl"] = BAAS_SERVER + "/file/render?filekey=" + $scope.similarCategory[i]["image"][0]["key"] + '&ask=' + ASK + '&osk=' + OSK + '&resize={"width":253,"height":180}';
                } else {
                    /*for default image*/
                    $scope.similarCategory[i]["imgurl"] = "/rest/file/render?filekey=530835834b43556925e9b1ee&ask=" + ASK + "&osk=" + OSK + '&resize={"width":253,"height":180}';
                }
            }
        }, function (jqxhr, error) {
        })
