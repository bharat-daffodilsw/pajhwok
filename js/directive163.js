pajhwokApp.directive('category', ["$compile", function ($compile, $scope) {
    return {
        ...
        template:"<div class='categories'>" +
            ...
            "<li ng-repeat= 'cat in category' id='accordion'>" +
            "<span ng-show='$index==0' class='cat-image'>" +
            "<img  class='catimg' src='images/election.png'>" +
            "</span>" +
            "<span ng-show='$index==1' class='cat-image'>" +
            "<img class='catimg' src='images/voteforpeace.png'>" +
            "</span>" +
            "<h4><span class='category-text cursor-pointer' ng-class='{selected_category: cat._id == selectedCategoryid.parent}' ng-click='setCategory(cat);fnviewEmployerView($index,cat);loadingMessage=true'>{{cat.en_title}}</h4>" +
            ...
            "<img class='catimg'src='images/voteforpeace.png'>" +
            "</span>" +
            "<span class='category-text cursor-pointer'ng-class='{selected_category: cat._id == selectedCategoryid.parent}'  ng-click='setCategory(cat);fnviewEmployerView($index,cat);loadingMessage=true'>{{cat.dr_title}}" +
            "</span>" +
            '<ul class="sub-ul"  ng-show="cat.viewRolesList">'+
            '<li ng-repeat="child in cat.childs"> ' +
            ...
            "<img class='catimg' src='images/voteforpeace.png'>" +
            "</span>" +
            "<span class='category-text cursor-pointer' ng-class='{selected_category: cat._id == selectedCategoryid.parent}' ng-click='setCategory(cat);fnviewEmployerView($index,cat);loadingMessage=true'>{{cat.ps_title}}</span>" +
            '<ul class="sub-ul"  ng-show="cat.viewRolesList">'+
            '<li ng-repeat="child in cat.childs"> ' +
...
pajhwokApp.directive('categoryList', [function ($scope) {
    return {
        restrict:"E",
        replace:true,
        template:"<span><select class='select' ng-show='uploadSelectedTrans == \"5304a09a476f9b995eb672f1\"' ng-model='selectcategory' ng-change='showSubCategory(selectcategory)'><option value=''>Select Category</option><option ng-repeat='cat in category' value='{{cat._id}}'>{{cat.en_title}}</option></select>" +
            "<select class='select' ng-show='uploadSelectedTrans == \"5304a0ec476f9b995eb672f3\"'  ng-model='selectcategory'  ng-change='showSubCategory(selectcategory)'><option value=''>Dr Select Category</option><option ng-repeat='cat in category' value='{{cat._id}}'>{{cat.dr_title}}</option></select>" +
            "<select class='select' ng-show='uploadSelectedTrans == \"5304a0cb476f9b995eb672f2\"'  ng-model='selectcategory'  ng-change='showSubCategory(selectcategory)'><option value=''>Ps Select Category</option><option ng-repeat='cat in category' value='{{cat._id}}'>{{cat.ps_title}}</option></select></span>"
...
pajhwokApp.directive('manageOptions', ["$compile", function ($compile, $scope) {
    return{
        restrict:"E",
        replace:true,
        template:'<ul class="down-menu-list"><li class="down-menu-item"><a>{{userSelectedLanguage["data"]["manage"]}}</a></li>' +
            ...
            '<li ng-click="setManageOpt(\'manageflaggednews\')" class="down-menu-items"><a >{{userSelectedLanguage["data"]["manageflag"]}}</a></li>' +
            '<li  ng-click="setManageOpt(\'manageunpublishnews\')" class="down-menu-items"><a >{{userSelectedLanguage["data"]["publishstories"]}}</a></li>' +
...
pajhwokApp.directive('navBar', ["$compile", function ($compile, $scope) {
    return {
        ...
        template:"<div class='nav'><span class='cities'><state></state></span>" +
            ...
            "<span class='right-nav'><ul class='view-bar'><li class='map-view view'><a href='#/'><img title='News Wall' src='images/1.png '></a></li>" +
...
pajhwokApp.directive('userArticle', ['$appService', '$compile', '$timeout', function ($appService, $compile, $timeout, $scope) {
    return{
        ...
        template:'<div ng-switch="newsVisible"><alert-popup ng-show="bShowAlertPopup"></alert-popup><p ng-switch-when="false" class="no-record">{{userSelectedLanguage.data.norecordfound}}</p><div ng-switch-when="true" class="single-video-wrapper">' +
            ...
            '</span>' +
            '</span></p>' +
            '<div addthis:url="http://afghansvote.af/#/news/{{news[0]._id}}" addthis:title="{{news[0].title}}" class="addthis_toolbox  addthis_default_style ">' +
...
pajhwokApp.directive("manageUnpublishNews", ['$appService', '$compile', function ($appService, $compile, $scope) {
    return {
        ...
        template:'<div ng-switch="unpublishedNewsVisible">' +
            '<alert-popup ng-show="bShowAlertPopup"></alert-popup><p ng-switch-when="false" class="no-record">{{userSelectedLanguage.data.norecordfound}}</p>' +
            ...
            '</p>' +
            '<p class="approve-btn">' +
            '<button class="video-approve-btn" ng-click="updateStatus(media,\'publish\')" ng-hide="media.published || media.rejected">{{userSelectedLanguage.data.approve}}</button>' +
//            '<label class="video-reject-hide-btn" ng-show="media.published || media.rejected">{{media.message}}</label>	 ' +
            '<button class="video-reject-btn" ng-click="updateStatus(media,\'reject\')" ng-hide="media.published || media.rejected">{{userSelectedLanguage.data.reject}}</button>' +
            '</p>' +
            '<p>' +
            '<label class="video-reject-hide-btn" ng-show="media.published || media.rejected">{{media.message}}</label> ' +
            '</p>' +
                ...
                post:function ($scope) {
                    ...
                    $scope.updateStatus = function (media, result) {
                        ...
                        $appService.save(query, ASK, OSK, null, function (dataUpdate) {
                            if (dataUpdate.response.update && dataUpdate.response.update.length > 0) {
                                $scope.alert.bShowAlertPopup = true;
                                $scope.alert.alertPopupMsg = userSelectedLanguage.data.successfullyupdated;
                            }
                            else if (dataUpdate.response.insert && dataUpdate.response.insert.length > 0) {
                                $scope.alert.bShowAlertPopup = true;
                                $scope.alert.alertPopupMsg =userSelectedLanguage.data.successfullyinserted;
                            }
                            else {
                                $scope.alert.bShowAlertPopup = true;
                                $scope.alert.alertPopupMsg =userSelectedLanguage.data.savingfail;
                            }
                        });
...
pajhwokApp.directive("manageUser", ['$appService', '$compile', function ($appService, $compile, $scope) {
    return {
        ...
        template:'<div><div ng-switch="visibleUserList"><p ng-switch-when="false" class="no-record" class="no-record">{{userSelectedLanguage.data.norecordfound}}</p>' +
            ...
            '<change-user-status ng-show="manageUserDisplay.bChangeRole"></change-user-status><alert-popup ng-show="alert.bShowAlertPopup"></alert-popup>' +
            ...
            '<span>' +
            '<label>{{userSelectedLanguage.data.email}}</label></span>' +
            '<span>' +
            '<a class="upper-arrow" >' +
            '<img src="images/w_arw_up.png"></a>' +
            '<a class="lower-arrow" >' +
            ...
            '<button class="change-role-btn" ng-click="changeUserRole(user)">{{userSelectedLanguage.data.ok}}</button>' +
            '</li>' +
            '<li class="data-element"><label class="date-created">{{user.__createdon}}</label></li>' +
            '<li class="data-element"><button class=" block-buttons"  ng-hide="user.userid.status" ng-click="changeStatus($index,user.userid._id)">{{userSelectedLanguage.data.activate}}</button>' +
            '<button class="block-buttons"   ng-show="user.userid.status" ng-click="changeStatus($index,user.userid._id)">{{userSelectedLanguage.data.deactivate}}</button>' +
            '</li>' +
            '</ul>' +
            '</div>' +
            '</div>' +
...
pajhwokApp.directive('similarmedia', function () {
    function link($scope, element, attrs) {
        if ($scope.newsData.displayType.image) {
            ...
            var html = "<a href='#/news/" + $scope.newsData._id + "'><div id='similarplayer" + $scope.$index + "'></div></a>"
            var fileUrl = $scope.newsData.fileUrl;
            var thumbnailUrl = $scope.newsData.thumbnail;
            var file_ext = $scope.getFileExtension($scope.newsData.media[0].name);
            element.append(html);
...
pajhwokApp.directive('mobileApp', [function ($scope) {
    return{
        restrict:"E",
        replace:true,
        template:'<div class="mobile-app"><p class="soon">{{userSelectedLanguage.data.comingsoon}}</p><div class=""><h3 class="left-heading">{{userSelectedLanguage.data.pajhwokmobappmsg}}</h3></div>' +
            '<div class="logo"><div class="apple-app"><a><img id="mobile-img" src="images/PANMAIN.png"></a></div>' +
            '<div class="google-app"><a><img id="mobile-img" src="images/google-play.png"></a></div></div></div>'
...
pajhwokApp.directive('specialMines', [function ($scope) {
    return{
        ...
        template:'<div class="special-mines"><div class=""><h3 class="left-heading">{{userSelectedLanguage.data.splminespages}}</h3></div>' +
            '<div class="logo"><img id="logo-white" src="images/logo-white.png"></div><div class="view-all"><div id="view-all">{{userSelectedLanguage.data.viewall}}</div>' +
            '<div class="right-arrow"><img src="images/arrow.png" class="view-all-arrow"></div></div></div>'
...
pajhwokApp.directive('mapBlock', [function ($scope) {
    return{
        restrict:"E",
        replace:true,
        template:'<div class="map-block"><div class="small-map-block"><a ng-click=setMapView()><img src="images/small-map.png"></a></div>' +
        ...
        compile:function () {
            return{
                post:function ($scope, iElement) {
                    ...
                    var html = '<div class="twitter-wrapper"><div class="twitter-block"><a width="263" height="700" class="twitter-timeline" href="https://twitter.com/afghansvote" data-widget-id="423920531142754304">Tweets by @afghansvote</a>' +
...
pajhwokApp.directive('loginBlock', [function ($scope) {
    return{
        restrict:"E",
        replace:true,
        template:'<div class="login-block"><span class="login"><div class="panel"><a ng-click="loginDetail.login=true" id="login_pop">{{userSelectedLanguage["data"]["login"]}}</a>' +
...
pajhwokApp.directive('similarCategory', ['$appService', '$compile', function ($appService, $compile, $scope) {
    return{
        ...
        template:'<div ng-switch="similarCategoryVisible"><div ng-switch-when="true" class="multiple-videos" ><div class="multiple-video-1" ng-repeat="newsData in similarCategory"><div class="only-video">' +
            '<a ng-click="setCountAndPath(newsData)"><span class="multiple-video-thumb" similarmedia ></span></a></div><div class="popular-lockup-content"><h3 class="popular-lockup-title"><a href="#/news/{{newsData._id}}" ><p class="popular-small-heading">' +
...
pajhwokApp.directive('afNews', ['$appService', function ($appService, $scope) {
    return{
        restrict:"E",
        replace:true,
        template:'<ul class="news-ul"><li class="meta-block"><h2 class="headline-block"><a href="#/news/{{newsData._id}}">' +
            '{{newsData.title}}</a></h2><p ng-show="\'5304a09a476f9b995eb672f1\'==selectedTrans" class="cat-block"><a>{{newsData["category"]["en_title"]}}</a></p>' +
            ...
            '<li class="sp-block"><div class="text-sp"><span class="cursor-pointer" ng-click="setCountAndPath(newsData)">{{newsData["commentcount"]}}&nbsp;{{userSelectedLanguage["data"]["comments"]}}</span></div></li><li class="sp-block"><div addthis:url="http://afghansvote.af/#/news/{{newsData._id}}" addthis:title="{{newsData.title}}" class="addthis_toolbox addthis_default_style addthis_32x32_style">' +
            '<a class="addthis_button_preferred_1"></a></div></li><li class="sp-block"><div addthis:url="http://afghansvote.af/#/news/{{newsData._id}}" addthis:title="{{newsData.title}}" class="addthis_toolbox addthis_default_style addthis_32x32_style">' +
            '<a class="addthis_button_preferred_2"></a></div></li>' +
            '<li ng-click="updateLikeCount(newsData,false)" ng-show="newsData.likestatus" class="sp-block cursor-pointer"><span><img src="images/like-blue.png">&nbsp;{{newsData["likecount"]}}</span></li>' +
            '<li ng-hide="newsData.likestatus" ng-click="updateLikeCount(newsData,true)"class="sp-block cursor-pointer"><span><img  src="images/Icon_Like.png">&nbsp;{{newsData["likecount"]}}</span></li>' +
            '<li ng-click="updateFlagCount(newsData,false)" ng-show="newsData.flagstatus" class="sp-block cursor-pointer"><span><img src="images/flag-blue.png" width="30px" height="30px"></span></li>' +
            '<li ng-hide="newsData.flagstatus" ng-click="updateFlagCount(newsData,true)" class="sp-block cursor-pointer"><span><img src="images/flag.png" width="30px" height="30px"></span></li>' +
            '</ul></li></ul>',
        compile:function () {
            return{
                pre:function ($scope) {
...
pajhwokApp.directive('mapNewsList', [function ($scope) {
    return{
        restrict:"E",
        replace:true,
        template:'<div class="left-news-wrapper"><div class="latest-stories"><h3>Latest Stories</h3></div><ul class="news-wrapper"><li class="recent-news-block" ng-repeat="newsData in mapRecentStories">' +
            '<small-news-list></small-news-list></li></ul><div style="text-align: center; height: 38px;"><a href="#/" class="readmore">{{userSelectedLanguage.data.viewall}} </a></div></div>'
...
pajhwokApp.directive('smallNewsList', [function ($scope) {
    return{
        restrict:"E",
        replace:true,
        template:'<div class="recent-wrapper"><div class="lockup-content"><h3 class="lockup-title"><a href="#/news/{{newsData._id}}" class="" >' +
            ...
            '</span>' +
            '<span class="deemphasized-text">{{newsData["__createdon"]}}</span></span></p><p id="truncate" class="recent-description">{{newsData.description}}</p><a href="#/news/{{newsData._id}}" class="readmore">{{userSelectedLanguage["data"]["readmore"]}}</a></div></div>'
