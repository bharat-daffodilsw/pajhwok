...
pajhwokApp.directive('topStoryMain', [function ($scope) {
    return{
        ...
        template:"<div ng-switch='topNewsVisible'><div ng-switch-when='true' class='bigger'><div class='video-only'><span class='video-thumb'>" +
            ...
            "<h3 class='lockup-title'><a href='#/news?newsid={{topNews._id}}'<p class='big-heading'>{{topNews.title}}</p></a></h3>" +
...
pajhwokApp.directive("manageUnpublishNews", ['$appService', '$compile', function ($appService, $compile, $scope) {
    return { 
        ...
        template:'<div ng-switch="unpublishedNewsVisible">' +
            ...
            '<ul class="news-ul">' +
            '<li class="meta-block">' +
            '<h2 class="headline-block">' +
            '<a href="#/news?newsid={{newsData._id}}">{{newsData.title}}</a>                ' +
            '</h2>' +
            ...
            '<input type="" class="edit-phone" placeholder="edit name here" ng-show="newsData.guest.name" ng-model="newsData.guest.name"><input class="edit-phone" type="" placeholder="edit phone here" ng-show="newsData.guest.name" ng-model="newsData.guest.phone">' +
...
pajhwokApp.directive('afNews', ['$appService', function ($appService, $scope) {
    return{
        restrict:"E",
        replace:true,
        template:'<ul class="news-ul"><li class="meta-block"><h2 class="headline-block"><a href="#/news?newsid={{newsData._id}}">' +
...
pajhwokApp.directive('mapNewsList', [function ($scope) {
    return{
		...
		'<thead><tr><th scope="col" class="title">{{userSelectedLanguage.data.title}}</th><th scope="col" class="location">{{userSelectedLanguage.data.location}}</th><th scope="col" class="date">{{userSelectedLanguage.data.date}}</th></tr></thead>'+
		'<tbody><tr ng-repeat="newsData in mapRecentStories">'+
		'<td><a ng-click="showLatestinPopup(newsData)">{{newsData.title| limitTo:100}}..</a></td><td >{{newsData.displayLocationName}}</td><td>{{newsData.__createdon| limitTo:10}}</td>'+
